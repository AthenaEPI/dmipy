...
def test_watson_dispersed_callaghan_kappa0(
    lambda_par=1.7e-9, diameter=8e-6, bvalue=1e9, mu=[0, 0], kappa=0
):
...
def test_bingham_dispersed_callaghan_kappa0(
    lambda_par=1.7e-9, diameter=8e-6, bvalue=1e9, mu=[0, 0], kappa=0, beta=0,
    psi=0
):
    ...
    E_bingham_callaghan = bingham_callaghan(scheme)
...
def test_gamma_distributed_callaghan(alpha=.1, beta=1e-5,
    ...
    for i, radius in enumerate(radii):
        E[i] = (
            ...
            callaghan(scheme, diameter=radius * 2)
        )
