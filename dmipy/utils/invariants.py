...
def fit_sh_to_data(data, vecs, sh_order=4, lambda_lb = 1e-03, sh_base = 'mrtrix'):
    ...
    if len(shape) > 4:
        raise ValueError(
            'len(data.shape) must be lower than 5')
    n_c = (sh_order + 1)*(sh_order + 2)//2
...
def real_gaunt4(l1,  l2,  l3,  l4,  mu1,  mu2,  mu3,  mu4,  RG):
    ...
    for l in range(0,N+1,1):
        for m in range(-l,l+1):
            ...
            #if condition(l,l2,l3,m,mu2,mu3) and condition(l1,l,l4,mu1,m,mu4):
            r += RG[c1] * RG[c2]
            ...
            for m1 in range(-l1,l1+1):
                for m2 in range(-l2,l2+1):
                    for m3 in range(-l3,l3+1):
                        ...
                        mtv += gaunt_real_sph_harm(m1,l1,0,0) * gaunt_real_sph_harm(m2,l2,0,0) * gaunt_real_sph_harm(m3,l3,0,0)* rg
                ...
                for m2 in range(-l2,l2+1):
                    for m3 in range(-l3,l3+1):
                        for m4 in range(-l4,l4+1):
                            ...
                            mtv += gaunt_real_sph_harm(m1,l1,0,0) * gaunt_real_sph_harm(m2,l2,0,0)*gaunt_real_sph_harm(m3,l3,0,0)*gaunt_real_sph_harm(m4,l4,0,0)*rg
                    ...
                    for m3 in range(-l3,l3+1):
                        for m4 in range(-l4,l4+1):
                            for m5 in range(-l5,l5+1):
                                ...
                                mtv += gaunt_real_sph_harm(m1,l1,0,0) * gaunt_real_sph_harm(m2,l2,0,0)*gaunt_real_sph_harm(m3,l3,0,0)*gaunt_real_sph_harm(m4,l4,0,0)*gaunt_real_sph_harm(m5,l5,0,0)*rg
