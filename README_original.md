## microstruktur
[![Build Status](https://travis-ci.org/uwescience/microstruktur.svg?branch=master)](https://travis-ci.org/uwescience/microstruktur)

Microstruktur is a template project for small scientific python projects. The
...
repository into your computer and follow the instructions at the [bottom of this page](#using-microstruktur-as-a-template).
...
`microstruktur/microstruktur.py`. This code provided in this file is intentionally rather
...
We place the module code in a file called `microstruktur.py` in directory called
`microstruktur`. This structure is a bit confusing at first, but it is a simple way
to create a structure where when we type `import microstruktur as sb` in an
interactive Python session, the classes and functions defined inside of the
`microstruktur.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the microstruktur module, the main classes defined at the bottom of
...
Either way, you can create a `microstruktur/data` folder in which you can
...
(typically, something like our `microstruktur/tests/test_microstruktur.py`). Within each
...
have one such test in `microstruktur/tests/test_microstruktur.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/microstruktur`),
and type:

    py.test microstruktur

This will exercise all of the tests in your code directory. If a test fails, you
will see a message such as:


    microstruktur/tests/test_microstruktur.py .F...
    ...
    microstruktur/tests/test_microstruktur.py:49: AssertionError
...
To document `microstruktur` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience/microstruktur/blob/master/doc/_static/RTD-advanced-conf.png)

 http://microstruktur.readthedocs.org/en/latest/
...
A `microstruktur/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/microstruktur) for the software. This
...
`microstruktur` against Python 2.7, Python 3.3 and Python 3.4). It includes steps
that need to be taken before installation of the software. For example,
installation of the software dependencies. For `microstruktur`, we use the
...
[here](https://github.com/uwescience/microstruktur/blob/master/scripts/Figure1.ipynb).


### Using `microstruktur` as a template
...
computer under the name you will want your project to have:
	...
	git clone https://github.com/uwescience/microstruktur smallish
	cd smallish
...
Next, you will want to move `microstruktur/microstruktur.py` to be called
`smallish/smallish.py`

	git mv microstruktur smallish
	git mv smallish/microstruktur.py smallish/smallish.py
	git mv smallish/tests/test_microstruktur.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `microstruktur` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `microstruktur` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `microstruktur` in them. Type
the following to see where all these files are:

	git grep microstruktur

You can replace `microstruktur` for `smallish` fastly with:

	git grep -l 'microstruktur' | xargs sed -i 's/microstruktur/smallish/g'
...
Edit all the mentions of `microstruktur` in the `microstruktur/__init__.py` file, and in
the `microstruktur/version.py` file as well.
